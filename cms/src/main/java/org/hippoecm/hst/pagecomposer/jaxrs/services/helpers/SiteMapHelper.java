...
public class SiteMapHelper extends AbstractHelper {
    ...
    private static final Logger log = LoggerFactory.getLogger(org.hippoecm.hst.pagecomposer.jaxrs.services.helpers.SiteMapHelper.class);
    ...
    public PageCopyContextImpl detach(final String mountId, final String sourceSiteMapItemUUID, final String targetSiteMapItemUUID, final String targetName) throws RepositoryException {
        String[] targetPathTokens = StringUtils.split(targetName, "/");
        final String relativeContentPath = StringUtils.removeEnd(StringUtils.removeStart(targetName, "/"), ".html");

        final String encodedName = getURLDecodedJcrEncodedName(targetPathTokens[targetPathTokens.length - 1]);

        PageCopyContextImpl pageCopyContext = copy(mountId, sourceSiteMapItemUUID, targetSiteMapItemUUID, encodedName);
        Node newSiteMapNode = pageCopyContext.getNewSiteMapItemNode();
        newSiteMapNode.setProperty(SITEMAPITEM_PROPERTY_RELATIVECONTENTPATH, relativeContentPath);
        return pageCopyContext;
    }


    /**
     * utility method to create a <strong>shallow copy</strong> of {@code siteMapItemId} : Shallow copy means that child
     * pages of {@code siteMapItemId} are not copied
     *
     ...
     * @return the {@link Node} of the created new siteMapItem
     ...
     * copied node will have name <code>targetName</code>. The node is copied <strong>without</strong> its children!
     */
    ...
    private void setSitemapItemProperties(final SiteMapItemRepresentation siteMapItem, final Node jcrNode) throws RepositoryException {
        ...
        if (siteMapItem.getPrimaryDocumentRepresentation() != null && siteMapItem.getPrimaryDocumentRepresentation().getPath() != null) {
            ...
            if (absPath.startsWith(rootContentPath + "/")) {
                setProperty(jcrNode, SITEMAPITEM_PROPERTY_RELATIVECONTENTPATH, absPath.substring(rootContentPath.length() + 1));
            } else if (absPath.equals("")) {
                removeProperty(jcrNode, SITEMAPITEM_PROPERTY_RELATIVECONTENTPATH);
            } else {
            ...
            setProperty(jcrNode, SITEMAPITEM_PROPERTY_RELATIVECONTENTPATH, siteMapItem.getRelativeContentPath());
        }
